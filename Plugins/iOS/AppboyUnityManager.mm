...
- (ABKSlideupShouldDisplaySlideupReturnType) shouldDisplaySlideup:(NSString *)message
{
  if (self.unitySlideupGameObjectName == nil) {
    NSLog(@"No slideup unity game object name defined. Not sending the slideup message to the Unity "
          ...
          "addSlideupListenerWithObjectName: callbackMethodName].");
    return ABKSlideupShouldIgnore;
  }
  if (self.unitySlideupCallbackFunctionName == nil) {
    NSLog(@"No slideup unity callback method name defined. Not sending the slideup message to the Unity "
          "Player. Please defined callback method name by calling [[AppboyUnityManager sharedInstance] "
          "addSlideupListenerWithObjectName: callbackMethodName].");
    return ABKSlideupShouldIgnore;
  }
  NSLog(@"Sending an Slideup message to %@.", self.unitySlideupGameObjectName);
...
- (void) registerApplication:(UIApplication *)application didReceiveRemoteNotification:(NSDictionary *)notification
{
  ...
  if (self.unityPushGameObjectName == nil) {
    NSLog(@"No push notification received unity game object name defined. Not sending the slideup message "
          "to the Unity Player. Please defined game object name by calling [AppboyUnityManager sharedInstance] "
          "addPushListenerWithObjectName: callbackMethodName].");
    return;
  }
  if (self.unityPushCallbackFunctionName == nil) {
    NSLog(@"No push notification received unity callback method name defined. Not sending the slideup message "
          "to the Unity Player. Please defined callback method name by calling [AppboyUnityManager sharedInstance] "
          "addPushListenerWithObjectName: callbackMethodName].");
    return;
  }
  NSLog(@"Sending a notification received message to %@.", self.unitySlideupGameObjectName);
